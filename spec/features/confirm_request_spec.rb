feature "Confirmation of requests" do
  scenario 'Owner of space can confirm a request' do
    ...
    expect(Space.first(name: space_name).booked).to eq(true)
    rejected_user=User.first(email: "han@brolo.com")
    expect(Request.first(user_id: rejected_user.id).active).to eq false
  end
  ...
  scenario 'Owner of space can confirm a request' do
    ...
    confirmed_request=Request.first(user_id: user_id, space_id: space_id)
    expect(current_path).to eq "/requests/index"
    expect(page).to have_content 'Space Booking Rejected'
    updated_request=Request.first(id: confirmed_request.id)
    expect(updated_request.confirmed).to eq false
    expect(updated_request.active).to eq false
